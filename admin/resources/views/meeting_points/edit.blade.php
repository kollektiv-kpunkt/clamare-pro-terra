<x-app-layout :title="__('Edit Entry') . ': ' . $meetingPoint->title">
    <x-app-container>
        ...
        <form action="{{route('meeting_points.update', $meetingPoint)}}" method="POST" class="cpt-form">
            @csrf
            @method('PUT')
            <x-form-group name="eventtype" :label="__('Entry Type')" :required="true" :value="old('eventtype',  $meetingPoint->eventtype)" :fullwidth="true" type="select" :options="['event', 'meetup', 'hike', 'bike']"/>
            ...
            <x-form-group name="link" :label="__('Link to chat or form')" :required="true" :value="old('link', $meetingPoint->link)" :fullwidth="true" type="url"/>
...
@if(($meetingPoint->eventtype == "hike" || $meetingPoint->eventtype == "bike") && $meetingPoint->polyline)
    let geoJson = {!! $meetingPoint->polyline !!};
    drawnItems = L.geoJson(geoJson).addTo(map);
    document.getElementById('creation-polyline').value = JSON.stringify(geoJson);
@else
